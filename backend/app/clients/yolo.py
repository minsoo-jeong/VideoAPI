...
class YoloClient(object):
    ...
    def _run_yolo(self, frames):
        ...
        client = YOLO(f"grpc://{settings.TRITON_HOST}:{settings.TRITON_PORT}/{self.MODEL_NAME}",
                      task='detect',
                      )
        tensors = [preprocess(f) for f in frames]

        predicts = client.predict(source=tensors,
                                  conf=0.5,
                                  iou=0.45,
                                  classes=[0, 1, 3],
                                  verbose=False,
        ...
        results = [json.loads(p.tojson()) for p in predicts]

        return results

    def __run_async(self, frames, redis):
        def callback(result: grpcclient.InferResult, error: grpcclient.InferenceServerException):

            info = dict(status='done')
            if error:
                info['error'] = str(error)
            else:
                preds = result.as_numpy(self.OUTPUT_NAME)
                batch_result = self.postprocess(preds, batch_array.shape[2:], frame_size[::-1])
                info['result'] = batch_result

            redis.set(_id, json.dumps(info))
        ...
        for idx in batch_indices:
            _id = generate_key()
            redis.set(_id, json.dumps({'status': 'processing'}))
            batch_array = np.array([self.preprocess(f) for f in frames[idx]])
            _input = grpcclient.InferInput(self.INPUT_NAME, batch_array.shape, "FP32")
            _input.set_data_from_numpy(batch_array)

            ctx = self.client.async_infer(model_name=self.MODEL_NAME,
                                          inputs=[_input],
                                          callback=callback,
                                          model_version=self.MODEL_VERSION,
                                          request_id=_id
    ...
    def __start_stream(self, callback):
        self.client.start_stream(callback)

    async def __run_async_v2(self, frames):
